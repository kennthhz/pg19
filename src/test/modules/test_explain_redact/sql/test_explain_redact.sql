...
--
-- T12: composite-type field names, and the named-argument label.
--
-- Landed after T13-T21a rather than in sequence.  It is here in the file, in
-- task order, because that is how it reads; it is in the tree now because T21b
-- turns expression output back on and a composite field name is printed inside
-- an expression.
--
-- FOUR ROUTES, AND WHICH ONE EACH FIXTURE ACTUALLY TAKES WAS MEASURED, NOT
-- ARGUED.  get_name_for_var_field() returns a field name from four separate
-- places and they share nothing but the caller, so a guard at one of them covers
-- none of the others.  The routes were distinguished by tagging each guard with a
-- NOTICE, running every candidate shape, and reading which tag fired -- the same
-- method T19 used, and for the same reason: two of T01's six predicted fixtures
-- did not reach the code they were written for.
--
--   route 1, RowExpr->colnames -- reached by fixture 02.  The colnames come from
--       a pulled-up subquery's target-list RESNAMES: pullup_replace_vars() turns
--       a whole-row reference to a flattened subquery into a RowExpr carrying
--       them.  That is the "sub-target-list resnames" source the task plan listed
--       as a separate fourth route; it is this one.  The OFFSET 0 matters -- it
--       stops the OUTER subquery being pulled up too, which would let
--       eval_const_expressions() fold FieldSelect-over-RowExpr down to the plain
--       column and print no field name at all.
--
--   route 2, the tuple descriptor's attname, mid-function -- reached by fixtures
--       01 and 04.  The ordinary case: a field of a named composite type.
--
--   route 3, get_rte_attribute_name() -- reached by fixture 03, and by nothing
--       simpler.  It needs a whole-row Var of type RECORD, which means a
--       whole-row reference to a CTE, subquery or function RTE, reached through
--       an enclosing Var so that a FieldSelect exists at all.
--
--   route 4, the tuple descriptor's attname at the very end, after the RECORD
--       drill-down -- SHIPS AS A GUARD, measured unreachable while redacting.  It
--       is live code: an unconditional probe fires it from pg_get_viewdef() on a
--       view whose subquery outputs a bare ROW(...).  It cannot fire from a plan
--       tree, because there rte->subquery is NULL and the RTE_SUBQUERY arm
--       recurses through the child plan's tlist instead of inspecting a
--       sub-target-list entry.  Parse-tree deparse has no pseudonym map (redacted
--       query text is deferred, requirements 3.1.1), so the guard is correct and
--       unexercised.
--
-- THE REQUIREMENTS' OWN WHOLE-ROW FIXTURE DOES NOT REACH ROUTE 3, and FR-93 is
-- corrected rather than transcribed.  "SELECT (zc).zsec_ssn FROM zsec_c zc"
-- produces no FieldSelect at all: ParseComplexProjection() short-circuits
-- (whole-row-Var).field straight to a plain Var for that column, so the query
-- deparses as an ordinary column reference and the fixture would have passed
-- while testing nothing.  Fixture 03 below is what reaches the route.
--
-- Each fixture carries a plain column reference in the same target list whose
-- pseudonym still prints, so a clean result cannot be confused with the
-- wholesale blanking T04 does.
--
-- A TABLE OF ITS OWN rather than two more columns on zsec_c, because a column
-- pseudonym's number follows the relation's attribute numbers: adding c_comp to
-- zsec_c renumbered c_when from t1_c10 to t1_c12 and rewrote four rows of the
-- T14 section, which is noise in a diff and not a change anyone meant.
CREATE TYPE zsec_t02.zsec_rec12 AS (zsec_fld_ssn text, zsec_fld_dob int);
CREATE TYPE zsec_t02.zsec_rec12b AS (zsec_fld_ssn int);
CREATE TABLE zsec_t02.zsec_cf (cf_first int,
                               cf_second text,
                               cf_comp zsec_t02.zsec_rec12,
                               cf_comp2 zsec_t02.zsec_rec12b);
-- plpgsql, so that inlining does not remove the FuncExpr the label hangs off.
CREATE FUNCTION zsec_t02.zsec_f12(zsec_argname int) RETURNS int
    LANGUAGE plpgsql AS 'BEGIN RETURN zsec_argname; END';
SELECT what,
       test_redact_deparse(qry, false) AS plain,
       test_redact_deparse(qry, true)  AS redacted
  FROM (VALUES
    ('01 field selection (route 2)',
     'SELECT cf_second, (cf_comp).zsec_fld_ssn FROM zsec_cf'),
    ('02 RowExpr colnames (route 1)',
     'SELECT cf_second, (q.s).zsec_fld_ssn FROM zsec_cf, (SELECT s FROM (SELECT cf_second AS zsec_fld_ssn FROM zsec_cf) s OFFSET 0) q'),
    ('03 whole-row RTE reference (route 3)',
     'WITH w AS MATERIALIZED (SELECT cf_second AS zsec_fld_ssn FROM zsec_cf) SELECT cf_second, (q.s).zsec_fld_ssn FROM zsec_cf, (SELECT s FROM w s OFFSET 0) q'),
    ('04 field of a pg_catalog composite',
     'SELECT c.relname, (q.a).attname FROM pg_class c, (SELECT a FROM pg_attribute a OFFSET 0) q'),
    ('05 named-argument label (FR-94)',
     'SELECT cf_second, zsec_t02.zsec_f12(zsec_argname => cf_first) FROM zsec_cf')
  ) AS t(what, qry)
 ORDER BY what COLLATE "C";
-- Name by name, because the pairs above are read by eye.  Each row: the real
-- name must appear in the plain deparse, must be gone from the redacted one, and
-- the pseudonym must be present -- the third clause is what stops the second
-- from being satisfied by an empty string.
--
-- Row 05 is the exception and says so: there is no pseudonym, because FR-94's
-- treatment OMITS the "name =>" decoration rather than renaming the label, so
-- the assertion is on the positional call shape instead.
SELECT what,
  ...
  FROM (VALUES
    ('01 field selection (route 2)', 'zsec_fld_ssn',
     'SELECT cf_second, (cf_comp).zsec_fld_ssn FROM zsec_cf',
     '\)\.fld[0-9]+'),
    ('02 RowExpr colnames (route 1)', 'zsec_fld_ssn',
     'SELECT cf_second, (q.s).zsec_fld_ssn FROM zsec_cf, (SELECT s FROM (SELECT cf_second AS zsec_fld_ssn FROM zsec_cf) s OFFSET 0) q',
     '\)\.fld[0-9]+'),
    ('03 whole-row RTE reference (route 3)', 'zsec_fld_ssn',
     'WITH w AS MATERIALIZED (SELECT cf_second AS zsec_fld_ssn FROM zsec_cf) SELECT cf_second, (q.s).zsec_fld_ssn FROM zsec_cf, (SELECT s FROM w s OFFSET 0) q',
     '\)\.fld[0-9]+'),
    ('05 named-argument label (FR-94)', 'zsec_argname',
     'SELECT cf_second, zsec_t02.zsec_f12(zsec_argname => cf_first) FROM zsec_cf',
     '^[^=]*$')
  ) AS t(what, realname, qry, pseudo)
 ORDER BY what COLLATE "C";
-- FR-94 IS A GUARD FOR A PATH EXPLAIN CANNOT REACH, and the plain half of row 05
-- above is what proves it: the label is already gone WITHOUT redaction, because
-- the parser resolves named notation to positional order and discards the
-- NamedArgExpr wrapper.  So "leaks_unredacted" is FALSE there, alone among the
-- rows, and that is the measurement rather than a failure.  Stated as its own
-- assertion so the false is not read as a bug.
SELECT test_redact_deparse(
         'SELECT zsec_t02.zsec_f12(zsec_argname => cf_first) FROM zsec_cf', false)
         AS named_arg_label_absent_even_unredacted;
-- FR-40 for field names: the same field reached twice in one record is one
-- pseudonym, a different field of the same type is a different one.  Asked with
-- the repeat at the END so that a per-call counter would be visible.
SELECT test_redact_deparse(
         'SELECT (cf_comp).zsec_fld_ssn, (cf_comp).zsec_fld_dob, (cf_comp).zsec_fld_ssn FROM zsec_cf',
         true) AS same_field_twice_one_pseudonym;
-- The documented consequence of keying on a hash of the NAME rather than on
-- (type, fieldno): two different composite types that happen to share a field
-- name share its pseudonym.  That is concealment either way, and it is pinned
-- here so the collapse is a decision on the record rather than a surprise.
SELECT test_redact_deparse(
         'SELECT (cf_comp).zsec_fld_ssn, (cf_comp2).zsec_fld_ssn FROM zsec_cf',
         true) AS same_name_two_types_one_pseudonym;
-- NEGATIVE CONTROL, AND IT DOES NOT SAY WHAT THE TASK BRIEF EXPECTED.  A field
-- of a pg_catalog composite type is NOT exempt: row 04 above prints "fld1" where
-- a pg_catalog COLUMN in the same target list still prints "relname".  That is
-- deliberate and it is the only self-consistent answer available.  Exemption is
-- a question about a catalog OID, and of the four routes only route 2 has one --
-- routes 1 and 3 hold a colnames list and an RTE.  Exempting where the OID
-- happens to be at hand would make one field of one type print as its real name
-- from one route and as fldN from another inside a single record, which is the
-- contradiction FR-40 exists to forbid.  FR-93 grants no exemption either, and
-- withholding more than required is the safe direction.
--
-- What the row still demonstrates is that the boundary is targeted rather than
-- blanket: "relname" survives as a column, "attname" does not survive as a
-- field, in one string.
SELECT test_redact_deparse(
         'SELECT c.relname, (q.a).attname FROM pg_class c, (SELECT a FROM pg_attribute a OFFSET 0) q',
         true) LIKE '%relname%' AS catalog_column_still_prints,
       test_redact_deparse(
         'SELECT c.relname, (q.a).attname FROM pg_class c, (SELECT a FROM pg_attribute a OFFSET 0) q',
         true) NOT LIKE '%attname%' AS catalog_composite_field_does_not;
...
--
-- T14: the remaining leaf sites -- EXTRACT's field, and four guards.
--
-- What is testable here was measured before anything was written, and the
-- measurement is what decided the test set.  test_redact_deparse() deparses the
-- top plan node's target list, so a site that only ever appears in a qual, or in
-- a child node, cannot be reached from here at all:
--
--   FR-98d, the EXTRACT field -- REACHABLE, and tested below.
--
--   FR-97, the cursor name -- NOT reachable.  A CurrentOfExpr only ever appears
--       in an UPDATE/DELETE qual.  Measured: the same UPDATE through this
--       harness returns the empty string without RETURNING, because a
--       ModifyTable has no target list of its own, and returns just the
--       RETURNING column with one; while plain EXPLAIN (COSTS OFF, VERBOSE) on
--       that statement shows "TID Cond: CURRENT OF zcur_secret" on the Tid Scan
--       below it.  The guard is right and the site is real, but nothing this
--       harness can ask will exercise it.
--
--   FR-99, nextval() -- NOT reachable either, one level down rather than in a
--       qual.  "INSERT INTO t (col) VALUES ('x')" puts nextval('t_id_seq') in
--       the target list of the Result node feeding the ModifyTable, and this
--       harness does not descend.  Measured: the empty string from the harness,
--       against "Output: nextval('zt_zid_seq'::regclass), 'x'::text, ..." on the
--       Result in plain EXPLAIN (VERBOSE).  Note this is the FR-61 shape going
--       untested, not a T14 edit: T10 already routed that call through
--       generate_relation_name(..., redact), and T14 changed nothing here.
--
--   FR-98c, the inference-element operator class, and get_parameter()'s
--       function/argument-name branch -- NOT reachable by construction rather
--       than by harness shape.  The planner reduces ON CONFLICT inference
--       elements to a list of index OIDs before EXPLAIN sees them, and the only
--       code that fills in argnames builds its own deparse context with
--       redaction off.  Both reasons are written out at the guards themselves.
--       No SQL is written for either.
--
-- The three unreachable ones ship as guards, which is exactly what T13 did with
-- get_tablefunc(), and they are carried into T21 as such.  Adding a harness
-- entry point to reach them was considered and rejected: an entry point that
-- exists only so that a test can pass does not show the production path works.
--
-- The fixtures below carry a non-redacted sibling column in the same target list
-- for the same reason T13's do.  Without it, "no secret in the output" cannot be
-- told apart from output that was blanked wholesale -- the way this work has
-- gone green four times while proving nothing.
ALTER TABLE zsec_t02.zsec_c ADD COLUMN c_when timestamptz;

-- The last two rows are negative controls, and they are the assertion that would
-- fail if someone later "fixed" the two normalization-form sites next door to the
-- EXTRACT one.  Those forms cannot carry user data: the form is a grammar
-- keyword, so "x IS 'anything' NORMALIZED" and "NORMALIZE(x, 'anything')" are
-- both syntax errors and only NFC/NFD/NFKC/NFKD can ever reach those lines.  They
-- stay on the same footing as the built-in operator names T10 keeps.  (The parser
-- also supplies the form when it is omitted, so both sites are always exercised
-- -- "x IS NORMALIZED" deparses as "x IS NFC NORMALIZED".  Leaving them alone had
...
SELECT what,
  ...
  FROM (VALUES
    ...
    -- The row that shows why the field is redacted at all.  It reads like a
    -- keyword, but it is an ordinary text constant and the grammar takes any
    -- string: the "unit not recognized" error is raised at execution, and EXPLAIN
    -- without ANALYZE does not execute, so the user's string is planned, deparsed
    -- and printed.  Unredacted, the marker is right there in the output.
    ('EXTRACT user text',
     'SELECT c_second, EXTRACT(''zsecdata-secret'' FROM c_when) FROM zsec_c'),
    -- A second SQL-syntax EXTRACT overload (interval, not timestamptz), so the
    -- fixture does not pass on one F_EXTRACT_* case while another goes unguarded.
    ('EXTRACT interval',
     'SELECT c_second, EXTRACT(day FROM (c_when - c_when)) FROM zsec_c'),
    -- Two fields in one target list: both become "?", so no reader can recover
    -- year-versus-month by comparing them.  That loss is accepted (see FR-98d).
    ('EXTRACT two fields',
     'SELECT c_second, EXTRACT(year FROM c_when), EXTRACT(month FROM c_when) FROM zsec_c'),
    ('IS NORMALIZED form kept',
...
-- FR-98d on the marker string itself, the same pair T13 uses for the PASSING
-- label.  Two booleans rather than one NOT LIKE, because a bare NOT LIKE is also
-- satisfied by a harness that returned nothing at all -- and because the field is
-- read straight off the Const datum, so FR-24's placeholder never reached it.
SELECT test_redact_deparse(
         'SELECT c_second, EXTRACT(''zsecdata-secret'' FROM c_when) FROM zsec_c',
         false) LIKE '%zsecdata-secret%' AS extract_field_leaks_unredacted;
SELECT test_redact_deparse(
         'SELECT c_second, EXTRACT(''zsecdata-secret'' FROM c_when) FROM zsec_c',
