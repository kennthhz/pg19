			   ...
			   const MemoryContextCounters *mem_counters)
{
	...
	/*
	 * Extensions can hook in here to add output of their own.  Under
	 * redaction they are skipped rather than trusted.
	 *
	 * The reason is that this hook hands the extension both the ExplainState
	 * and the whole plan tree, so it can print anything it likes -- including
	 * by calling the expression printer directly, which knows nothing about
	 * redaction.  Nothing this file does can constrain that.  pg_overexplain,
	 * in our own tree, is the demonstration: from this hook it prints the
	 * entire range table, every column name included.
	 */
...
ExplainPrintSettings(ExplainState *es)
{
	...
	/*
	 * The Settings section lists the planner parameters whose values differ
	 * from the defaults.  search_path is one of them, so the section really
	 * can print the user's schema names.  It is dropped entirely.
	 */
	if (es->redact)
		return;
...
ExplainPrintPlan(ExplainState *es, QueryDesc *queryDesc)
{
	...
	/*
	 * Query Identifier is a hash of the statement with its literal values
	 * stripped out, produced by an algorithm that ships with the server. That
	 * makes it useless as a way of hiding the statement: anyone holding the
	 * log can guess a statement, hash it the same way, and see whether the
	 * numbers match.  Publishing the hash therefore comes close to publishing
	 * the statement for anybody able to guess it, so it is dropped.
	 *
	 * Records still need to be traceable back to what produced them.  That is
	 * done with a random token generated per record, which is a later task.
	 */
	if (!es->redact &&
		es->verbose && queryDesc->plannedstmt->queryId != INT64CONST(0) &&
...
{
	/*
	 * This property is the statement exactly as the client sent it, so it
	 * names every table, column and function the statement mentions --
	 * whether or not the plan itself would have revealed them -- and it
	 * carries any values written directly into the SQL.  On its own it undoes
	 * the whole feature, and there is no partial version of it worth keeping.
	 */
	if (es->redact)
		return;
...
ExplainQueryParameters(ExplainState *es, ParamListInfo params, int maxlen)
{
	...
	/*
	 * When a client runs a prepared statement, this property logs the values
	 * it bound to the placeholders -- so unlike the rest of a plan, which is
	 * built from schema names, what appears here is the application's actual
	 * data.  In a real deployment it is the account number or the email
	 * address itself.  Redaction omits the property completely.
	 *
	 * Omitting it completely, rather than keeping the placeholder names and
	 * dropping only the values, needs a word of explanation, because "$1, $2"
	 * looks harmless.  It is not quite.  It reveals how many parameters the
	 * statement had, including any the plan never used.  And on a system with
	 * log_parameter_max_length set to 0 this property is already suppressed
	 * entirely, so a redaction mode that printed "$1, $2" would show a reader
	 * more than they can see today.  Switching redaction on must never reveal
	 * something that leaving it off concealed.
	 *
	 * Returning here, ahead of the code below rather than around the property
	 * itself, is also deliberate.  BuildParamLogString() turns every bound
	 * value into text and assembles the lot into one buffer, which means
	 * calling each parameter type's output function, and those can be
	 * user-defined.  Leaving early neither builds that buffer nor runs that
	 * code, so the values are never rendered into memory only to be
	 * discarded.
	 *
	 * See FR-22 and D10 in the requirements document for the full argument.
	 */
	if (es->redact)
		return;
			...
			ExplainState *es)
{
	...
	switch (nodeTag(plan))
	{
		...
		case T_CustomScan:
			...
			/*
			 * A custom scan provider supplies its own display name, chosen by
			 * whoever wrote the extension, so it may well name a product or a
	...
	switch (nodeTag(plan))
	{
		...
		case T_ForeignScan:
			...
			/*
			 * A foreign data wrapper prints the SQL it sends to the remote
			 * server, which is the user's query written against the user's
			 * remote tables.
			 */
			if (!es->redact)
		...
		case T_CustomScan:
			{
				...
				if (plan->qual)
					show_instrumentation_count("Rows Removed by Filter", 1,
											   planstate, es);
				if (css->methods->ExplainCustomScan && !es->redact)
	...
	/*
	 * Skipped for the same reason as explain_per_plan_hook earlier: an
	 * extension handed the ExplainState can print whatever it chooses.
	 */
	if (explain_per_node_hook && !es->redact)
					  ...
					  ExplainState *es)
{
	...
	for (j = 0; j < mtstate->mt_nrels; j++)
	{
		...
		/*
		 * Give FDW a chance if needed.  Skipped under redaction for the same
		 * reason as the scan-side callback: the wrapper prints its own SQL.
		 */
