...
- **Does not sanitize the log-record envelope.** The csvlog/jsonlog/syslog
  ...
  *(rev. query-id fix, post-T23: one field of the redacted record's own line
  is now handled by auto_explain — the query identifier, as printed by `%Q`
  and by the csvlog/jsonlog `query_id` field — because auto_explain emits that
  line itself and can clear the identifier for it; see FR-37. The rest of the
  envelope, and the query identifier on every other line, remain out of
  scope.)*
...
| FR-2 | When enabled, redaction applies to every plan auto_explain logs: top-level statements and nested statements, all log formats, all log levels, sampled and unsampled-per-`sample_rate` alike. *(rev. context leak, post-T23: "every plan" was met and the nested record still leaked, through what its `ereport()` carries besides the message. Under `log_nested_statements` PL/pgSQL's error-context callback attached the nested statement's SQL, verbatim, and the function's name as the record's `CONTEXT`. Closed by FR-100. The FR-2 test in 002_redact.pl now scans every line of every entry auto_explain writes, not only the plan lines `plan_record()` returns, which is why it had not seen this.)* |
| FR-3 | Core `EXPLAIN` gains an option `REDACT` (`EXPLAIN (REDACT ON)` / `EXPLAIN (REDACT TRUE)`), producing the same redaction contract interactively, primarily so the behavior is regression-testable without loading auto_explain. *(rev. T22: one clause of the contract is **not** reachable this way, and the "primarily so" claim should be read with that exception. The allowlist is `auto_explain.redact_allow_schemas` (FR-51) and core has no equivalent option, so an interactive `EXPLAIN (REDACT)` always runs with an empty allowlist: `ExplainState.redact_allow_schemas` stays NIL for every core caller. Allowlist behavior is therefore verified by the engine's unit tests (`src/test/modules/test_explain_redact`, which passes a list to `explain_redact_create()` directly) and by TAP (`contrib/auto_explain/t/004_redact_allow_schemas.pl`), and cannot be exercised from `src/test/regress/sql/explain_redact.sql` at all. Adding a second, core-side GUC would close the gap and would be a change to FR-51, which names exactly one option; it was not done under T22.)* |
...
| FR-37 | `Query Identifier`: the queryId is a deterministic, publicly specified hash of the parse tree — an offline dictionary attack can confirm that a specific guessed query ran *(rev. security review)* | **omitted** from redacted records *(rev. query-id fix, post-T23: the omission now also covers the log envelope of the redacted record's own line. `%Q` in `log_line_prefix` and the `query_id` field of csvlog/jsonlog read the backend's current identifier when a line is written, so with `compute_query_id = on` they printed, beside the plan, the value the plan withheld. auto_explain saves the identifier, clears it with `pgstat_report_query_id(0, true)` around the one `ereport()` that writes the redacted record, and restores the saved value with `pgstat_report_query_id(saved, true)` in `PG_FINALLY`; that line shows `0`. Every other line, the DEBUG1 companion entry (FR-76) included, is unchanged.)* *(rev. query-id gaps, post-T23: both gaps that revision left open are closed. (1) `track_activities` turned off partway through a statement (`SET LOCAL` in a function) made `pgstat_report_query_id()` a no-op while `pgstat_get_my_query_id()` — what `%Q` and the log formats read — kept returning the outer statement's identifier, so the record still printed it. The clear and the restore now go through a new core function, `pgstat_set_my_query_id()`, which stores exactly the value given whatever `track_activities` says and does nothing without a backend status entry; no existing signature changes, and auto_explain does not touch the status entry's write protocol itself. (2) auto_explain's own FR-75/FR-51/FR-73 warnings, written during the statement just before its record, carried its identifier; the allowlist and `log_extension_options` ones leaked it with nothing else in the log to account for it. Every warning is now written with the identifier cleared the same way. Both sites use one pair of helpers, `hide_query_id()` / `restore_query_id()`, with the restore in `PG_FINALLY` at each; both run only under redaction (FR-62). The companion entry still keeps its identifier.)* |
...
| FR-51 | A list-valued option `auto_explain.redact_allow_schemas` exempts additional schemas (e.g. trusted extension schemas such as `pg_trgm`) from redaction. Allowlisting a schema exempts **everything later created in it**; documentation must warn against allowlisting user schemas such as `public`. Objects exempted by the allowlist print their real names; everything else still redacts. *(rev. T22, as landed: the option is auto_explain-only, so the allowlist is not reachable from the interactive `EXPLAIN (REDACT)` path and not testable from the regression suite -- see the T22 note on FR-3. The list reaches the pseudonym engine as `ExplainState.redact_allow_schemas`, set by auto_explain next to `es->redact` and read once when the map is created; core's creation site remains the only one. Parsed by `SplitIdentifierString()` in a `PGC_SIGHUP` check hook, which is what makes the names catalog-normalised as `explain_redact.c`'s exact comparison requires, and cached in parsed form as GUC extra. The documentation warning this requirement asks for is also a runtime one: `public` in the list is reported once per session, at LOG, through the same channel as the FR-75 envelope conflicts, because a `PGC_SIGHUP` assign hook can only write to the server log detached from any record. Exemption is by **namespace and OID-keyed object**: an exempt relation's column names print in full too, while its aliases stay pseudonymous and its constants stay `?` -- the allowlist exempts names, never data.)* |
...
| ID | Requirement |
|---|---|
| FR-70 | `errhidestmt(true)` remains in effect for redacted auto-explain records so the raw statement is not attached to the same log entry. *(rev. context leak, post-T23: necessary, not sufficient. `STATEMENT` is one of several things an `ereport()` carries in addition to its message, and `errhidestmt()` touches only that one. `CONTEXT` is the same class and did carry the nested statement; FR-100 adds `errhidecontext(true)` beside `errhidestmt(true)` on the record and on the warnings. The rest of the class, checked: `DETAIL` appears on the warnings only and is fixed text; no `HINT` is set; `QUERY` (`internal_query`) is set by the in-core callbacks — SPI, PL/pgSQL, SQL functions — only when the report has an error position, which these never do; `LOCATION` under `log_error_verbosity = verbose` and `BACKTRACE` when `backtrace_functions` names the reporting function carry C source locations and addresses only. A third-party error-context callback could set any of these, and `emit_log_hook` sees the report before any flag is applied; neither is auto_explain's to govern.)* |
| FR-75 | When redaction is enabled, auto_explain emits a one-time-per-session `LOG` notice if any concurrently active logging setting would place unredacted user information into the same log stream: `log_statement`, `log_min_duration_statement`, or a `log_line_prefix` containing `%q` *(rev. security review: envelope/statement channels; see §3)* *(rev. query-id fix, post-T23: `%q` is removed from this list and its warning from the code. The premise was wrong: `%q` prints nothing — it only marks where the prefix stops for non-session processes — so the warning fired on a harmless prefix, the test harness's default among them, and said nothing about the escape that does carry withheld information, `%Q`. That one is handled at the record instead (FR-37) and is not warned about.)* |
...
| FR-73 | `auto_explain.log_extension_options` may activate extension explain output. In redacted records such options are ignored (per FR-25) and a `LOG` notice is emitted stating that extension explain options were skipped due to redaction. *(rev. extension-options latch, post-T23: the notice was written on every redacted record while the option was set, although the documentation said once per session and `warn_redaction_conflict()` exists to keep these configuration notices out of every record. It is now once per session, latched by a file-scope flag like the FR-51 allowlist warning's. The parameter's assign hook re-arms it, so a superuser who `SET`s the option partway through a session is told again, once; the hook only clears the flag, so off mode is unchanged (FR-62). Tests: 005, exactly one notice across three redacted records in each of two sessions, and exactly two, one on each side of the `SET`, in a session that changes it.)* |
...
**FR-100 — `CONTEXT` on redacted records and on the warnings.** [V]
*(rev. context leak, post-T23.)* Fixture: `zq_nested_then_qid()` in
005_redact_query_id.pl, run with `log_nested_statements`, stderr and jsonlog.
Assert, each absence against a presence in the same configuration:
the nested record's jsonlog entry has no `context` field and its stderr entry
no `CONTEXT:` line — while with redaction off the same record's context names
the nested SQL and the function, and with redaction on the companion entry
still does; no `zq_` fixture name appears anywhere in any redacted record's
whole entry, every jsonlog field and every stderr line — while the same scan
finds both names in the companions; the FR-75 warnings, written inside the
nested statement's context (the entry after the last of them is that
